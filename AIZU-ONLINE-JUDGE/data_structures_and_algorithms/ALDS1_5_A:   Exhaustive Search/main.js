...
var main = function () {
    ...
    try {
        ...
        // step3 mの要素数qを受け取る
        var q = parseInt(lines[2].trim());
        // step4 mの要素を受け取る
        var m = lines[3].trim().split(' ').map(function (str) { return parseInt(str); });
        checkCorrectArray(m, q);
        var INDEX = 0;
        for (var i = 0; i < m.length; i++) {
            var result = solve(INDEX, m[i], n, A) ? "yes" : "no";
            console.log(result);
        }
    }
    catch (e) {
...
//                || solve(3, -5) => false
var solve = function (index, targetSum, inputTargetArrayLength, targetArray) {
    ...
    var withoutCurrent = solve(index + 1, targetSum, inputTargetArrayLength, targetArray);
    //recursive2 現在の要素を使用して次へ
    var withCurrent = solve(index + 1, targetSum - targetArray[index], inputTargetArrayLength, targetArray);
    var res = withoutCurrent || withCurrent;
    return res;
};
/**
 * 配列の要素数が正しいかチェックする
 */
var checkCorrectArray = function (arr, inputIndex) {
