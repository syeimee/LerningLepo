 ...
 */
var koch = function (depth, startingPoint, endPoint) {
    if (depth === 0) {
        return;
    }
    // step2 始点と終点を1:2,2:1に内分点s,tを求める。ただしｎ=0のとき、始点(0,0)、終点(100,0)とする。
    var sx = (startingPoint[0] * 2 + endPoint[0]) / 3;
    var tx = ((startingPoint[0] + endPoint[0] * 2) / 3);
    var sy = (startingPoint[1] * 2 + endPoint[1]) / 3;
    var ty = ((startingPoint[1] + endPoint[1] * 2) / 3);
    // step3 uを回転移動で求める
    var u = rotate60Degrees([tx, ty], [sx, sy]);
...
/*    Y = y' + s_y
/*/
var rotate60Degrees = function (t, s) {
    var theta = (Math.PI / 3); // 60度 = π/3ラジアン
    var rotatedX = (t[0] - s[0]) * Math.cos(theta) - (t[1] - s[1]) * Math.sin(theta) + s[0];
    var rotatedY = (t[0] - s[0]) * Math.sin(theta) + (t[1] - s[1]) * Math.cos(theta) + s[1];
