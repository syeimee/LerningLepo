  ...
  const todaysLessons = useMemo(() => {
    ...
    return lessons
      .filter((lesson) => {
        const lessonDate = new Date(lesson.date)
        lessonDate.setHours(0, 0, 0, 0)
        return lessonDate.getTime() === today.getTime() && lesson.teacherId === teacherId
      })
      .map((lesson) => {
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card>
              <CardContent>
                ...
                <Typography variant="h3">{teachersCount}</Typography>
              </CardContent>
            </Card>
          </Grid>
              ...
              return (
                ...
                >
                  <ListItemText
                    ...
                    secondary={`生徒: ${lesson.student.lastName} ${lesson.student.firstName}${
                      userRole === 'ADMIN' ? ` | 講師: ${lesson.teacher.lastName} ${lesson.teacher.firstName}` : ''
                    }`}
                  />
                </ListItem>
              )
